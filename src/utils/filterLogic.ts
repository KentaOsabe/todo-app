...
export const filterByCompletion = (todos: Todo[], status: FilterState['completionStatus']): Todo[] => {
...
export const filterByCategories = (todos: Todo[], categoryIds: string[]): Todo[] => {
  if (categoryIds.length === 0) return todos
  return todos.filter(todo => categoryIds.includes(todo.categoryId || ''))
}

export const filterByTags = (todos: Todo[], tags: string[], condition: 'any' | 'all'): Todo[] => {
