...
- 明示指示がない限り、コミット・Push・PR作成は実施しない。
  - デフォルト動作: 変更案（差分/パッチ）を提示し、承認後に実行。
  - 実行指示の例: 「コミットして」「Pushして」「PR更新して」。
- レビュー対応中か否かに関わらず、上記ルールを徹底する。
- テスト/Lint/Formatはローカル実行・結果共有までに留め、コミット・Pushは行わない（指示がある場合を除く）。
- 例外が必要な場合も、事前に合意を得てから実行する。
